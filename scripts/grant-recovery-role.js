...
async function main() {
    ...
    if (!process.env.RECOVERY_ADDRESS) {
        throw new Error("RECOVERY_ADDRESS not found in .env file");
    }

    // Contract address
    const contractAddress = process.env.CONTRACT_ADDRESS;
    
    // Address to receive the RECOVERY_ROLE
    const addressToGrant = process.env.RECOVERY_ADDRESS;

    console.log("Starting grant RECOVERY_ROLE process...");
    ...
    try {
        ...
        // Calculate RECOVERY_ROLE hash
        const RECOVERY_ROLE = ethers.keccak256(ethers.toUtf8Bytes("RECOVERY_ROLE"));
        console.log("RECOVERY_ROLE hash:", RECOVERY_ROLE);
        ...
        const hasRole = await contract.hasRole(RECOVERY_ROLE, addressToGrant);
        if (hasRole) {
            console.log("\nAddress already has RECOVERY_ROLE");
            return;
        }

        // Grant the role
        console.log("\nGranting RECOVERY_ROLE...");
        const tx = await contractWithSigner.grantRole(RECOVERY_ROLE, addressToGrant, {
            gasLimit: 200000,
            gasPrice: 250000000
        });
        ...
        const roleGranted = await contract.hasRole(RECOVERY_ROLE, addressToGrant);
        console.log("\nRole granted successfully:", roleGranted);

        if (roleGranted) {
            console.log("RECOVERY_ROLE successfully granted!");
        } else {
            console.log("Failed to grant RECOVERY_ROLE!");
        }
